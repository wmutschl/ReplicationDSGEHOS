function X=ldivide(M,X)
%LDIVIDE method for KronProd. 
%
% Y=ldivide(M,X) or Y=M.\X
%
%At least one of M or X is a KronProd object. The other may be a scalar in which case
%Y is an appropriately scaled KronProd object.
%     
...
%full(M).\full(X).
  ...
  if isnum(M) %implies X is KronProd
    ...
    X.scalarcoeff=full(double(M.\X.scalarcoeff)); 
    return;
  ...
  elseif isa(M,'KronProd')  &  isa(X,'KronProd')
   ...
   elseif X.maxdim==M.maxdim
    ...
    error('Undefined ldivide operation involving KronProd');
  end
